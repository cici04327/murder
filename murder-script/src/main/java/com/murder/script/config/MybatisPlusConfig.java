...
public class MybatisPlusConfig {
    ...
    public MybatisPlusInterceptor mybatisPlusInterceptor() {
        log.info("开始配置 MyBatis Plus 分页插件");
        ...
        try {
            ...
            // 添加到拦截器（使用反射方法）
            interceptor.getClass().getMethod("addInnerInterceptor", 
                Class.forName("com.baomidou.mybatisplus.extension.plugins.inner.InnerInterceptor"))
                .invoke(interceptor, paginationInterceptor);
            
            log.info("✅ MyBatis Plus 分页插件配置成功 - 数据库: MYSQL, 最大限制: 1000");
        } catch (Exception e) {
            log.error("❌ 配置 MyBatis Plus 分页插件失败: {}", e.getMessage(), e);
        }
